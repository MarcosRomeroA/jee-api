...
use App\Contexts\Web\User\Domain\Exception\UserAlreadyExistsException;
...
final class UserCreatorTest extends TestCase
{
    ...
    public function testItShouldCreateAUser(): void
    {
        ...
        $password = new PasswordValue(password_hash('password123', PASSWORD_BCRYPT));
        ...
        // Simular que no existe usuario con ese username
        $this->repository
            ->expects($this->once())
            ->method('findOneBy')
            ->willReturn(null);
        ...
        $this->repository
            ->expects($this->once())
            ->method('save')
            ->with($this->callback(function ($user) use ($id) {
    ...
    public function testItShouldUpdateExistingUser(): void
    {
        ...
        $this->creator->__invoke($id, $firstname, $lastname, $username, $email, null);
    }
}
