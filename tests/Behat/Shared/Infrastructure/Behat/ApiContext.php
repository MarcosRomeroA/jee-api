...
final class ApiContext extends RawMinkContext
{
    ...
    public function theResponseShouldContainPaginationStructure(): void
    {
        ...
        if (!is_int($response['metadata']['total']) || $response['metadata']['total'] < 0) {
            throw new RuntimeException('The "total" field must be a non-negative integer');
        }

        if (!is_int($response['metadata']['count']) || $response['metadata']['count'] < 0) {
            throw new RuntimeException('The "count" field must be a non-negative integer');
        }

        if (!is_int($response['metadata']['limit']) || $response['metadata']['limit'] < 0) {
            throw new RuntimeException('The "limit" field must be a non-negative integer');
        }

        if (!is_int($response['metadata']['offset']) || $response['metadata']['offset'] < 0) {
            throw new RuntimeException('The "offset" field must be a non-negative integer');
        }
    }
     ...
     */
    public function theResponseShouldHavePropertyWithValue(string $property, string $value): void
    {
        $response = json_decode($this->sessionHelper->getResponse(), true);
        ...
        if (array_key_exists('data', $response) && is_array($response['data'])) {
            if (array_key_exists($property, $response['data'])) {
                $actualValue = is_string($response['data'][$property]) ? $response['data'][$property] : json_encode($response['data'][$property]);
                $found = true;
            }
        }
        ...
        if (!$found) {
            ...
            $actualValue = is_string($response[$property]) ? $response[$property] : json_encode($response[$property]);
        }
